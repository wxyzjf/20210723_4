...
execute ${etlvar::UTLDB}.ETL_UTILITY.TRUNCATE_TBL(p_table_schema=>'${etlvar::TMPDB}',p_table_name=>'B_RETENT_CORP_COMM_002A_T');
execute ${etlvar::UTLDB}.ETL_UTILITY.TRUNCATE_TBL(p_table_schema=>'${etlvar::TMPDB}',p_table_name=>'B_RETENT_CORP_COMM_002A02_T');
execute ${etlvar::UTLDB}.ETL_UTILITY.TRUNCATE_TBL(p_table_schema=>'${etlvar::TMPDB}',p_table_name=>'B_RETENT_CORP_COMM_002A03_T');
execute ${etlvar::UTLDB}.ETL_UTILITY.TRUNCATE_TBL(p_table_schema=>'${etlvar::TMPDB}',p_table_name=>'B_RETENT_CORP_COMM_002B01_T');
execute ${etlvar::UTLDB}.ETL_UTILITY.TRUNCATE_TBL(p_table_schema=>'${etlvar::TMPDB}',p_table_name=>'B_RETENT_CORP_COMM_002C01_T');
execute ${etlvar::UTLDB}.ETL_UTILITY.TRUNCATE_TBL(p_table_schema=>'${etlvar::TMPDB}',p_table_name=>'B_RETENT_CORP_COMM_002C02_T');
execute ${etlvar::UTLDB}.ETL_UTILITY.TRUNCATE_TBL(p_table_schema=>'${etlvar::TMPDB}',p_table_name=>'B_RETENT_CORP_COMM_002D_T');
execute ${etlvar::UTLDB}.ETL_UTILITY.TRUNCATE_TBL(p_table_schema=>'${etlvar::TMPDB}',p_table_name=>'B_RETENT_CORP_COMM_002_T');
prompt 'b_retent_corp_comm_002A_t [Preparing billing first serval bill inv list within the valid ld period]';

 insert into ${etlvar::TMPDB}.b_retent_corp_comm_002a_t(
         rpt_mth
        ,comm_mth
        ,case_id        
        ,ld_inv_num
...
insert into  ${etlvar::TMPDB}.b_retent_corp_comm_002a02_t
(
         rpt_mth
        ,comm_mth
        ,case_id 
...
select  
        ...
        ,nvl(max(decode(t.bill_inv_rnk,4,t.bill_inv_subr,3,t.bill_inv_subr)),' ') as map_bill_subr
        ,nvl(max(decode(t.bill_inv_rnk,4,t.bill_inv_cust,3,t.bill_inv_cust)),' ') as map_bill_cust
        ,nvl(max(decode(t.bill_inv_rnk,4,t.bill_inv_rnk,3,t.bill_inv_rnk)),0) as map_bill_inv_rnk
...
from ${etlvar::TMPDB}.b_retent_corp_comm_002a_t t
...
insert into  ${etlvar::TMPDB}.b_retent_corp_comm_002a03_t
(
         rpt_mth
        ,comm_mth
        ,case_id
...
from(
    Select 
        ...
        ,sh.transaction_type rebate_type
            ...
            ,'SAL_LED_HIST' 
    from ${etlvar::TMPDB}.b_retent_corp_comm_002a02_t t
...
insert into ${etlvar::TMPDB}.B_RETENT_CORP_COMM_002B01_T
(       rpt_mth
        ,comm_mth
        ,case_id
        ,ld_inv_num
...
insert into ${etlvar::TMPDB}.B_RETENT_CORP_COMM_002C01_T
(
        rpt_mth 
        ,comm_mth
        ,case_id
...
left outer join ${etlvar::ADWDB}.RETENT_CORP_COMM_OVERR_HIST ov
        on t.ld_inv_num = ov.inv_num
           and ov.COMM_MTH = &comm_mth;


prompt 'B_RETENT_CORP_COMM_002C02_T [ Mapping the sales info ]';
insert into ${etlvar::TMPDB}.B_RETENT_CORP_COMM_002C02_T
(
        rpt_mth 
        ,comm_mth
        ,case_id
...
select
        ...
        ,t.salesman_cd
        ,nvl(r.sale_team,' ')
from  ${etlvar::TMPDB}.B_RETENT_CORP_COMM_002C01_T t
left outer join ${etlvar::ADWDB}.retent_corp_comm_salesman r
...
insert into ${etlvar::TMPDB}.b_retent_corp_comm_002d_t
(             
        case_id
       ,mkt_cb_amt
        ,json_rmk
...
insert into ${etlvar::TMPDB}.B_RETENT_CORP_COMM_002_T
(
    rpt_mth
    ,comm_mth
    ,case_id
...
from ${etlvar::TMPDB}.B_RETENT_CORP_COMM_001_T t
left outer join ${etlvar::TMPDB}.B_RETENT_CORP_COMM_002A03_T inv
        on t.case_id = inv.case_id
left outer join ${etlvar::TMPDB}.B_RETENT_CORP_COMM_002B01_T hs
        on t.case_id = hs.case_id
left outer join ${etlvar::TMPDB}.B_RETENT_CORP_COMM_002C02_T sa
        on t.case_id = sa.case_id
left outer join ${etlvar::TMPDB}.B_RETENT_CORP_COMM_002D_T mc
